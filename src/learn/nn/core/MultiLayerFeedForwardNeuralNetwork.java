...
abstract public class MultiLayerFeedForwardNeuralNetwork extends FeedForwardNeuralNetwork {
	...
	public void backprop(Example example, double alpha) {
		...
		InputUnit[] inputs = this.getInputUnits();
		LogisticUnit temp = new LogisticUnit();

		//resets delta values before every call;
		//without this, the values of delta can be very small (10^-4)
		for(int i = 1; i < layers.length; i ++) {
			NeuronUnit[] resetDelta = getLayerUnits(i);
			for(int j = 0; j < layers[i].length; j++) {
				resetDelta[j].delta = 0;
				//				System.out.println("0 = " + getLayerUnits(i)[j].delta);
			}
		}
		
		
		//calculate delta[j] for output nodes
		...
		//must be a logic error in here
		for(int l = layers.length - 2; l > 0; l--) {

			NeuronUnit[] currentLayer = getLayerUnits(l);
...
//					System.out.println("test:" + temp.activationPrime(currentLayer[i].getInputSum(nextLayer[j].delta)));
		...
		//update weights			
		//weights are actually getting updated now =)
		//use dump() to test and see for yourself that the values of the weights are getting updated
		for(int l = 1; l < layers.length; l++) {
			NeuronUnit[] current = getLayerUnits(l);
			for(int j = 0; j < layers[l].length; j++) {
				for(int i = 0; i < layers[l-1].length; i++) {
					if(l == 1) {
						//System.out.println(current[j].getWeight(j));
						current[j].setWeight(i, current[j].getWeight(i) + alpha * temp.activation(current[j].getInputSum()) * current[j].delta);
						//System.out.println("----Updating layer " + (l) + " node " + (j) + " weight " + i + " with: " + current[j].getWeight(i));
						//dump();

					}else {
						//NeuronUnit[] previous = getLayerUnits(l-1);
						current[j].setWeight(i,current[j].getWeight(i) + alpha * temp.activation(current[j].getInputSum()) * current[j].delta);
						//System.out.println("Updating layer " + (l) + " node " + (j) + " weight " + i + " with: " + current[j].getWeight(i));
						//dump();
					}
